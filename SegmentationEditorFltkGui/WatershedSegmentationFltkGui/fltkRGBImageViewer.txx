...
RGBImageViewer<ImagePixelType,OverlayPixelType>
::RGBImageViewer()
{
  glSliceView = 0;
  CreateGUI();
...
template <class ImagePixelType, class OverlayPixelType>
typename RGBImageViewer<ImagePixelType, OverlayPixelType>::ColorTablePointer
RGBImageViewer<ImagePixelType,OverlayPixelType>
...
::Synchronize(void) 
{
  ...
  double iwMin  = ((int)(glSliceView->iwMin()*pow((float)10.0, -(float)b)))/pow((float)10.0,(float)-b);
  double iwMax  = ((int)(glSliceView->iwMax()*pow((float)10.0, -(float)b)))/pow((float)10.0,(float)-b);
...
void
RGBImageViewer<ImagePixelType,OverlayPixelType>
::CreateGLColorSliceView( Fl_Group * group, Fl_Gl_Window * original )
{
  ...
  this->glSliceView = new GLColorSliceViewType(x,y,w,h,label);
...
::UpdateClickedPoints(void)
{
  ...
  for(int i=numPoints-1; i>=0; i--)
  {
    ...
    ClickPoint point;
    glSliceView->getClickedPoint((unsigned int)i, point);
