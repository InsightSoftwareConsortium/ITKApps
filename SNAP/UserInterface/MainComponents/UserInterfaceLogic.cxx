...
void 
UserInterfaceLogic
::InitColorMap(bool resetCurrentAndDrawOverLabels) 
{
  ...
  // Only reset the current and draw-over colors if asked to
  if(resetCurrentAndDrawOverLabels)
    {
    m_GlobalState->SetDrawingColorLabel(m_ColorMap[defcol]);
    m_GlobalState->SetOverWriteColorLabel(m_ColorMap[0]);
    }
  
  m_InDrawingColor->value(m_GlobalState->GetDrawingColorLabel());
  switch(m_GlobalState->GetCoverageMode())
    {
    case PAINT_OVER_ALL : m_InDrawOverColor->value(0); break;
    case PAINT_OVER_COLORS : m_InDrawOverColor->value(1); break;
    default : m_InDrawOverColor->value(m_GlobalState->GetOverWriteColorLabel() + 2); break;
    };
  ...
  if(m_SystemInterface->FindRegistryAssociatedWithFile(
    ...
    if(m_DlgRestoreSettings->GetRestoreSettings())
      ...
      if(m_DlgRestoreSettings->GetRestoreLabels())
        {
        // The parameter 'false' means that the drawing and overlay labels
        // will not be reset.  This will reset the color label table
        this->OnSegmentationLabelsUpdate(false);
...
::OnLoadLabelsAction() 
{
  ...
  try 
    ...
    m_Driver->ReadLabelDescriptionsFromTextFile(file);    
