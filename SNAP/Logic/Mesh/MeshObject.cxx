...
::GenerateMesh()
{
  ...
  if (m_GlobalState->GetSnakeActive() && 
    ...
    {
    // We are going to get the current level set from SNAP
    ...
    // Compute hte mesh only for the current segmentation color
    ...
    if(!m_GlobalState->GetSnakeActive())
      ...
      else if(snapData->IsSnakeInitializationLoaded())
        {
        meshPipeline->SetImage(snapData->GetSnakeInitialization()->GetImage());
        }
      else 
        {
