...
if git config remote.stage.url >/dev/null; then
  ...
  git remote add stage git://itk.org/stage/ITKApps.git || \
    die "Could not add the topic stage remote."
  git config remote.stage.pushurl git@itk.org:stage/ITKApps.git
fi
...
EOF
  ...
  git ls-remote git@itk.org:stage/ITKApps.git refs/heads/master || \
    die "SSH test to git@itk.org failed. You may need to request access at:

https://www.kitware.com/Admin/SendPassword.cgi

Note that push access to the stage/ITKApps is separate to Gerrit.
"
